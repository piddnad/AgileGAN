...
from criteria import id_loss, w_norm, moco_loss, kl_loss
from configs import data_configs
from datasets.images_dataset import ImagesDataset
from criteria.lpips.lpips import LPIPS
from models.agilegan import EncoderModel
...
class Coach:
	def __init__(self, opts):
		...
		# Initialize network
		self.net = EncoderModel(self.opts).to(self.device)
		...
		if self.opts.kl_lambda > 0:
			self.kl_loss = kl_loss.NormalKLLoss().to(self.device).eval()
	...
	def train(self):
		...
		while self.global_step < self.opts.max_steps:
			for batch_idx, batch in enumerate(self.train_dataloader):
				...
				y_hat, latent, mu, logvar = self.net.forward(x, return_latents=True)
				loss, loss_dict, id_logs = self.calc_loss(x, y, y_hat, latent, mu, logvar)
				loss.backward()
				self.optimizer.step()
	...
	def validate(self):
		...
		for batch_idx, batch in enumerate(self.test_dataloader):
			...
			with torch.no_grad():
				x, y = x.to(self.device).float(), y.to(self.device).float()
				y_hat, latent, mu, logvar = self.net.forward(x, return_latents=True)
				loss, cur_loss_dict, id_logs = self.calc_loss(x, y, y_hat, latent, mu, logvar)
	...
	def calc_loss(self, x, y, y_hat, latent, mu, logvar):
		loss_dict = {}
		loss = 0.0
		id_logs = None
		if self.opts.id_lambda > 0:
		...
		if self.opts.kl_lambda > 0:
			loss_kl = self.kl_loss(mu, logvar)  # TODO
